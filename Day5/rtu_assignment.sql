...
select * from sensors where sensor_status='DISABLED';

-- 2.	List sensors which are having required value less than current value
select * from sensors where req_val<curr_val;
...
select * from sensors where isnull(curr_val);

-- 5.	List all sensors which are of type TEMPERATURE & PRESSURE.
select * from sensors where sensor_type='TEMPERATURE' or sensor_type='PRESSURE';

-- 6.	List sensors with rtu_id 102 and sensor type pressure and required value greater than 30.
select * from sensors where rtu_id='R102' and sensor_type='PRESSURE' and req_val>30;
